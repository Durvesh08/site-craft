...
export async function assembleHTML(
  ...
  const PREAMBLE = [
    ...
    // ── Pre-defined UI Helper Components ──
    `function CountingNum({ target, suffix = '', prefix = '', duration = 1.2 }) {`,
    `  var ref = useRef(null);`,
    ...
    `  }, children);`,
    `}`,
    `function Scene3D({ type = 'floating-geometry', primaryColor = '#6366f1', secondaryColor = '#8b5cf6', accentColor = '#a78bfa' }) {`,
    `  var canvasRef = useRef(null);`,
    `  useEffect(function() {`,
    `    if (!canvasRef.current || !window.THREE) return;`,
    `    var THREE = window.THREE;`,
    `    var renderer = new THREE.WebGLRenderer({ canvas: canvasRef.current, alpha: true, antialias: true });`,
    `    renderer.setSize(window.innerWidth, window.innerHeight);`,
    `    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));`,
    `    var scene = new THREE.Scene();`,
    `    var camera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 1000);`,
    `    var pColor = new THREE.Color(primaryColor);`,
    `    var sColor = new THREE.Color(secondaryColor);`,
    `    var aColor = new THREE.Color(accentColor);`,
    `    var fId = 0;`,
    ...
    `    if (type === 'floating-geometry') {`,
    `      camera.position.z = 6;`,
    `      var light = new THREE.PointLight(pColor, 2, 20);`,
    `      light.position.set(3, 4, 5);`,
    `      scene.add(light);`,
    `      scene.add(new THREE.AmbientLight(0x1e1b4b, 0.8));`,
    `      var mat = new THREE.MeshStandardMaterial({ color: pColor, wireframe: true, transparent: true, opacity: 0.55 });`,
    `      var ico = new THREE.Mesh(new THREE.IcosahedronGeometry(1.5, 1), mat);`,
    `      ico.position.set(-1.5, 0.5, 0);`,
    `      scene.add(ico);`,
    `      var tor = new THREE.Mesh(`,
    `        new THREE.TorusGeometry(1, 0.35, 16, 60),`,
    `        new THREE.MeshStandardMaterial({ color: sColor, wireframe: true, transparent: true, opacity: 0.45 })`,
    `      );`,
    `      tor.position.set(2, -0.5, -1);`,
    `      scene.add(tor);`,
    `      var box = new THREE.Mesh(`,
    `        new THREE.BoxGeometry(0.8, 0.8, 0.8),`,
    `        new THREE.MeshStandardMaterial({ color: aColor, wireframe: true, transparent: true, opacity: 0.5 })`,
    `      );`,
    `      box.position.set(0, -1.8, 1);`,
    `      scene.add(box);`,
    `      var tick = function(t) {`,
    `        t = t || 0;`,
    `        fId = requestAnimationFrame(tick);`,
    `        ico.rotation.y = t * 0.0004;`,
    `        ico.rotation.x = t * 0.0002;`,
    `        tor.rotation.x = t * 0.0005;`,
    `        tor.rotation.z = t * 0.0003;`,
    `        box.rotation.y = t * 0.0006;`,
    `        box.rotation.x = t * 0.0004;`,
    ...
    `        var angle = (i / count) * Math.PI * 12;`,
    `        var r = (i / count) * 5;`,
    `        pos[i * 3] = Math.cos(angle) * r + (Math.random() - 0.5) * 0.6;`,
    `        pos[i * 3 + 1] = (Math.random() - 0.5) * 1.2;`,
    `        pos[i * 3 + 2] = Math.sin(angle) * r + (Math.random() - 0.5) * 0.6;`,
    `        var t = i / count;`,
    `        var c = pColor.clone().lerp(sColor, t);`,
    `        col[i * 3] = c.r;`,
    `        col[i * 3 + 1] = c.g;`,
    `        col[i * 3 + 2] = c.b;`,
    `      }`,
    ...
    `        geo,`,
    `        new THREE.PointsMaterial({ size: 0.04, vertexColors: true, transparent: true, opacity: 0.75 })`,
    `      );`,
    `      scene.add(pts);`,
    `      var tick = function() {`,
    `        fId = requestAnimationFrame(tick);`,
    ...
    `      };`,
    `      tick();`,
    `    } else if (type === 'product-stage') {`,
    `      renderer.shadowMap.enabled = true;`,
    `      camera.position.set(0, 3, 8);`,
    `      camera.lookAt(0, 0, 0);`,
    `      var ambient = new THREE.AmbientLight(0xffffff, 0.3);`,
    `      scene.add(ambient);`,
    `      var key = new THREE.SpotLight(pColor, 3, 20, Math.PI / 6, 0.4);`,
    `      key.position.set(-4, 8, 4);`,
    `      key.castShadow = true;`,
    `      scene.add(key);`,
    `      var fill = new THREE.PointLight(sColor, 1.5, 15);`,
    `      fill.position.set(4, 2, 3);`,
    `      scene.add(fill);`,
    `      var rim = new THREE.PointLight(aColor, 1, 12);`,
    `      rim.position.set(0, -2, -4);`,
    `      scene.add(rim);`,
    `      var platform = new THREE.Mesh(`,
    `        new THREE.CylinderGeometry(3, 3, 0.18, 64),`,
    `        new THREE.MeshStandardMaterial({ color: 0x1e1b4b, metalness: 0.6, roughness: 0.3 })`,
    `      );`,
    `      platform.position.y = -1.5;`,
    `      platform.receiveShadow = true;`,
    `      scene.add(platform);`,
    `      var ring = new THREE.Mesh(`,
    `        new THREE.TorusGeometry(3, 0.04, 8, 64),`,
    `        new THREE.MeshStandardMaterial({ color: pColor, emissive: pColor, emissiveIntensity: 0.8 })`,
    `      );`,
    ...
    `      };`,
    `      tick();`,
    `    } else if (type === 'waveform-terrain') {`,
    `      camera.position.set(0, 4, 8);`,
    `      camera.lookAt(0, 0, 0);`,
    `      scene.add(new THREE.AmbientLight(0x1e1b4b, 0.5));`,
    `      var glow = new THREE.PointLight(pColor, 2, 20);`,
    `      glow.position.set(0, 3, 2);`,
    `      scene.add(glow);`,
    `      var W = 60, H = 60;`,
    `      var geo = new THREE.PlaneGeometry(14, 14, W - 1, H - 1);`,
    `      geo.rotateX(-Math.PI / 2);`,
    `      var origPos = new Float32Array(geo.attributes.position.array);`,
    `      var mat = new THREE.MeshStandardMaterial({ color: pColor, wireframe: true, transparent: true, opacity: 0.5 });`,
    ...
    `        for (var i = 0; i < W * H; i++) {`,
    `          var x = origPos[i * 3], z = origPos[i * 3 + 2];`,
    `          pos[i * 3 + 1] = Math.sin(x * 0.8 + t * 0.0008) * 0.7 + Math.cos(z * 0.6 + t * 0.0006) * 0.5;`,
    `        }`,
    ...
    `      };`,
    `      tick();`,
    `    } else if (type === 'aurora-sphere') {`,
    `      camera.position.z = 5;`,
    `      var al = new THREE.AmbientLight(pColor, 0.4);`,
    `      scene.add(al);`,
    `      var pl1 = new THREE.PointLight(sColor, 2.5, 15);`,
    `      pl1.position.set(3, 3, 3);`,
    `      scene.add(pl1);`,
    `      var pl2 = new THREE.PointLight(aColor, 1.8, 15);`,
    `      pl2.position.set(-3, -2, 2);`,
    `      scene.add(pl2);`,
    `      var geo = new THREE.SphereGeometry(2, 64, 64);`,
    `      var origPos = new Float32Array(geo.attributes.position.array);`,
    `      var mat = new THREE.MeshStandardMaterial({ color: sColor, wireframe: true, transparent: true, opacity: 0.4 });`,
    ...
    `          var nx = origPos[i], ny = origPos[i + 1], nz = origPos[i + 2];`,
    `          var len = Math.sqrt(nx * nx + ny * ny + nz * nz);`,
    `          var disp = 1 + 0.18 * Math.sin(nx * 2.5 + t * 0.001) * Math.cos(ny * 2.5 + t * 0.0008) * Math.sin(nz * 1.5 + t * 0.0012);`,
    `          pos[i] = (nx / len) * 2 * disp;`,
    `          pos[i + 1] = (ny / len) * 2 * disp;`,
    `          pos[i + 2] = (nz / len) * 2 * disp;`,
    `        }`,
    `        geo.attributes.position.needsUpdate = true;`,
    `        geo.computeVertexNormals();`,
    ...
    `      renderer.dispose();`,
    `    };`,
    `  }, [type, primaryColor, secondaryColor, accentColor]);`,
